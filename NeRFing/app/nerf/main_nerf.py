...
if cfg.pretrained and cfg.trainer.model_format == "full":
    ...
    blas1 = instantiate(cfg.blas, pointcloud=pointcloud)
    grid1 = instantiate(cfg.grid, blas=blas1)
    blas2 = instantiate(cfg.blas, pointcloud=pointcloud)
    grid2 = instantiate(cfg.grid, blas=blas2)
    nef = instantiate(cfg.nef, grid=[grid1, grid2])    # NeRF uses a grid as the backbone
