...
class MultiviewTrainer(BaseTrainer):
    ...
    def step(self, data):
        ...
        if self.epoch < 250:
            self.pipeline.nef.cur_res = 64
        elif self.epoch < 500:
            self.pipeline.nef.cur_res = 128
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                for idx, (img, ray_o, ray_d) in tqdm(enumerate(zip(imgs, ray_os, ray_ds))):
                    ...
                    rb.rgb = torch.clamp(rb.rgb, 0.0, 1.0)
                    ...
                    if self.cfg.save_valid_imgs:
                        if (rb.rgb[...,:3].max() <= 1.0 and rb.rgb[...,:3].min() >= -0.0):
                            print("WARNING, CLAMPING FOR SAVING IMAGE")
                        write_png(os.path.join(valid_log_dir, out_name + ".png"), rb.cpu().image().byte().rgb)
                        write_png(os.path.join(valid_log_dir, out_name + "_gt.png"), (gts[...,:3]*255).byte().cpu())
        log_text = 'EPOCH {}/{}'.format(self.epoch, self.max_epochs)
