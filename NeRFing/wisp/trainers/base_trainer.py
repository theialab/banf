...
class ConfigBaseTrainer:
    ...
    scheduler_milestones : Tuple[float, ...] = (0.25, 0.8) #(0.5, 0.75, 0.9)
...
class BaseTrainer(ABC):
    ...
    def init_optimizer(self):
        ...
        for name in params_dict:
            if 'decoder' in name:
                if '64' in name:
                    decoder_params64.append(params_dict[name])
                elif '128' in name:
                    decoder_params128.append(params_dict[name])
                else:
                     assert 0, "decoder name not found"
            
            elif 'grid' in name:
                if 'grid2' in name:
                    grid_params128.append(params_dict[name])
                else:
                    grid_params64.append(params_dict[name])
            else:
        ...
        params.append({"params": decoder_params64, "lr": lr, "eps": eps, "weight_decay": weight_decay})
        params.append({"params": decoder_params128, "lr": lr*1/self.cfg.scheduler_gamma*0.1, "eps": eps, "weight_decay": weight_decay})

        params.append({"params": grid_params64, "eps": eps, "lr": lr * self.cfg.grid_lr_weight})
        params.append({"params": grid_params128, "eps": eps, "lr": lr * self.cfg.grid_lr_weight*1/self.cfg.scheduler_gamma*0.1})

        params.append({"params": rest_params, "eps": eps, "lr": lr})

        max_steps = len(self.train_dataset) * self.cfg.max_epochs
        milestone_iters = [int(max_steps * x) for x in self.cfg.scheduler_milestones]
    ...
    def render_snapshot(self):
        ...
        out = self.render_random_snapshot()
        for key in log_buffers:
            if out.get(key) is not None:
                self.tracker.log_image(f'{key}_random', out[key].T, self.epoch)
    
    def render_random_snapshot(self):
        self.pipeline.eval()
        lod_idx = self.pipeline.nef.grid.num_lods - 1
        
        # sample ranodm position on sphere
        origin = torch.rand(3)
        ...
        if self.pipeline.tracer.bg_color == 'black' and out.rgb.shape[-1] > 3:
            bg = torch.ones_like(out.rgb[..., :3])
            out.rgb[..., :3] += bg * (1.0 - out.rgb[..., 3:4])
        out.rgb[..., :3] = torch.clamp(out.rgb[..., :3], 0, 1)
        out = out.image().byte().numpy_dict()
        return out
